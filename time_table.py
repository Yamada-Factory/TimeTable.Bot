...
def get_time_table(date):
    ...
    # デフォルトの時間割を読み込む
    with open(fs.TIME_TABLE, 'r', encoding = 'utf-8') as f:
        table = []
        reader = csv.reader(f)
        for row in reader:
            table.append(row)
...
# なければ空リスト
def get_event(date):
    events = access("SELECT * FROM event WHERE date = '{}'".format(get_date(date)))
    out = list()
    for event in events:
...
def access(query):
    ...
    f = logging.FileHandler('./bot_log/sql-access-'+datetime.datetime.today().strftime('%Y-%m')+'.log')
